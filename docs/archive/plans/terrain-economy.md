...
2026-07-11 (see `docs/reference/balance.html` §II/§V/§X for the analysis behind it). This is the
...
- The engine's terrain deck (`src/game/data.ts` `TERRAIN_DECK`) currently maps
  ...
  food, and the spec (`docs/reference/v0.1-rules-spec.md`) admits the hill row was an improvised
...
6. **Luxury goods are the scaling tier of happiness** — above temples, below nothing:
   ...
   - **Folded into `docs/plans/luxury-goods.md` (2026-07-23)** — that file is now the single
...
> and the full reasoning are in `docs/archive/roadmap-appendix.md` Q24/Q25; this section is the
> build contract.
...
`docs/reference/simulation.md`) and compare the aggregated balance report:
...
- ~~Do coastal tiles (when they land) also carry slots, or are they pure feature tiles?~~ —
  **now appendix Q31, moved into `docs/plans/luxury-goods.md` §7**; the lean is unchanged
...
- ~~Where exactly diminishing luxury duplicates and the 3-per-player cap live in state~~ —
  **superseded**: the claim model (a board-level `LuxuryClaim` list with `active` +
  `suppressedTurns`) is specified in `docs/plans/luxury-goods.md` §4.1, and the duplicates
...
settled principles into `docs/reference/v0.1-rules-spec.md` and the balance ledger.
