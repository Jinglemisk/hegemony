...
- Follow the roadmap (docs/roadmap.md) and drive work through its Q&A workbench (docs/archive/roadmap-appendix.md).
...
- Keep the balance ledger current (docs/reference/balance.html).
...
- Assembly 3-B is BUILT (spec + build record: docs/archive/plans/assembly-politicians.md) — STILL OWED:
...
- Terrain follow-ups (pinned in docs/archive/plans/terrain-economy.md): the **constrained shuffle**
...
-- **docs/plans/luxury-goods.md is the one-stop shop** (rewritten 2026-07-23): the plan,
...
- Two-panel remainders (docs/archive/plans/two-panel.md) — now UNBLOCKED by 3-B:
...
-- **Deep-link finer targets:** a specific building/entry, not just the chapter (needs concept-ids).
   These double as the codex-rulebook deep-link destinations (docs/archive/plans/codex-rules.md piece 4).
...
- Bot next lever: co-tune `evaluateSmart` weights via `--tune-patch` — the beam-vs-baselines run
  (docs/reports/simulation/2026-07-19-beam-vs-baselines.md) showed beam beats one-ply smart but LOSES to greedy
