...
> This brief is a *plan*, not a contract. The live rules contract stays `docs/reference/v0.1-rules-spec.md`;
> the single backlog stays `docs/archive/todo.md`. Fold accepted items into those.
...
- **The target is about to move.** Luxury goods + trade reset every relative value. This is
  exactly why your own `docs/archive/todo.md:10-27` parks the Buildings pass and Market/bank pass to
...
- **Sim:** execute `docs/archive/notes/simulation-plan.md` Track A — rebalance `SMART_POP_WEIGHT` (`policies.ts:225`)
