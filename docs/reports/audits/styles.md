...
Scope: `src/styles.css` (barrel, `@import` only) + 14 slice files in `src/styles/` (4,502 lines total). Color values also flow in from TS: `src/ui/resourceVisuals.ts`, `src/game/data.ts` (`PLAYER_COLORS`), `src/components/board/topbar/SeasonDial.tsx` (`SEASON_TINT`), `src/components/ResourceGrid.tsx` (unrest override).
