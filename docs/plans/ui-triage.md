...
Suites: 527 tests, 95 parity — the three added with CHRON-2, which walks every
authored card name so a new card cannot quietly reopen it.
...
| CHRON-2  | major   | chronicle | **The chronicle mis-glyphed card titles.** `AnnotatedText` matched a WORD and could not tell it was standing inside a name, so "Theron resolved Granary Rats" drew the _granary building_ glyph beside a card about vermin — a structure you can build, pictured next to a misfortune. Wrong information, not noise. Closed by lifting the authored titles out of the text before the tokenising pass; 15 of the 88 authored names were affected. | **fixed** |
| TICKER-1 | minor   | dock      | The dock printed the raw log line, `-5 wood` in flat ink, while the chronicle a few hundred pixels away printed `-5 Wood` in oxblood with a timber glyph. One event, two voices, both on screen. Closed by rendering the ticker through `<AnnotatedText links={false} />`; the dock keeps its eight tab stops.                                                                                                                                    | **fixed** |
| CER-1    | blocker | event     | **Fate cards had no voice**, because `EventCard` had no field to hold one — `card.text` _is_ the rules sentence, so the ceremony could print its one fact twice or say nothing. Closed by an optional authored `flavor`, 39 lines written, and the ceremony's flavour slot restored. Ten of the lines were recovered rather than invented: a voice fragment welded onto the front of a rules sentence, lifted out.                                | **fixed** |
| SHELL-3  | major   | board     | **The compass rose was bisected by the dock and half-covered by the season dial at every width** — unfixable in CSS because the rose was painted into a `center / cover` plate, so its position moved with the viewport while the dial's is fixed. Closed by `BOARD-BG-1`: the ornament came off the ground, and the collision cannot happen to a texture.                                                                                        | **fixed** |
