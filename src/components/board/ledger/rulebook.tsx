    ...
    return (
      <div className="compendiumStack">
        ...
        <Entry id={anchor("victory", "cards")} title="The cards & their minimums">
          <Note>
            A card can only be held once its measure passes a floor — set so nothing is winnable at
        ...
        <ul className="compendiumCostList">
          {terrainSummary(G).map((row) => (
            <li
              ...
              key={row.kind}
              style={row.resource ? resourceCssVars(row.resource) : undefined}
            >
              <span className="compendiumCostLabel">
                {row.resource ? (
                  <ResourceIcon resource={row.resource} className="marketRowIcon" />
                ) : null}{" "}
            ...
            <DefRow
              ...
              term={
                <span style={resourceCssVars(resource)} className="ruleResTerm">
                  <ResourceIcon resource={resource} className="marketRowIcon" />{" "}
  ...
  Body: ({ G }) => (
    <div className="compendiumStack">
      <Entry id={anchor("population", "classes")} title="The three classes">
        <Note>
          Every settlement is worked by three kinds of pop. Each earns and eats a fixed amount every
          income, straight from the ruleset:
        </Note>
        <DefList>
      ...
      <Entry id={anchor("settlements", "kinds")} title="The three kinds">
        <ul className="compendiumCostList">
          {(["capital", "city", "colony"] as SettlementKind[]).map((kind) => {
            const rule = G.ruleset.settlements[kind];
            return (
...
const ladder: RuleChapter = {
  ...
  Body: ({ G }) => {
    const promotes = Object.entries(G.ruleset.ladder.promoteCosts) as Array<
      [PopType, Partial<typeof G.ruleset.growPopCosts.slaves>]
    >;
    const demotes = Object.entries(G.ruleset.ladder.demoteCosts) as Array<
      [PopType, Partial<typeof G.ruleset.growPopCosts.slaves>]
    >;
    return (
      <div className="compendiumStack">
        <Entry id={anchor("ladder", "grow")} title="Growing">
          ...
          <DefList>
            {(
              Object.entries(G.ruleset.growPopCosts) as Array<
                [PopType, Partial<typeof G.ruleset.growPopCosts.slaves>]
              >
            ).map(([pop, cost]) => (
    ...
    return (
      <div className="compendiumStack">
        ...
        <Entry id={anchor("assembly", "propose")} title="Proposing">
          ...
          <DefList>
            <DefRow term="Draw">
              Pay <strong>{rules.drawCost}</strong> influence to draw from a politician{" "}
            ...
            <DefRow term="Repeal">
              ...
              instead. It is voted like any other resolution — undoing a law is as political as
              passing one.
            </DefRow>
        ...
        <Entry id={anchor("assembly", "vote")} title="Voting">
          <Note>
            The ballot is voted one resolution at a time, in turn order, <strong>openly</strong> —
            every vote is visible as it lands, so the last seat to speak on a close card is a
            kingmaker.
          </Note>
          <DefList>
