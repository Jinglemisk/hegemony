...
import { enumerateLegalCommands, popCompositions, transition } from "./legalMoves";
...
function applyTestCommand(G: HegemonyState, playerID: PlayerId, command: GameCommand) {
  const result = transition(G.definition, G, playerID, command);
  if (result.ok) {
    Object.assign(G, structuredClone(result.state));
  }
  return result;
}

/** Every enumerated command must apply cleanly — the load-bearing invariant. */
function expectSound(G: HegemonyState, playerID: PlayerId) {
  const moves = enumerateLegalCommands(G, playerID);

  for (const move of moves) {
    const clone = structuredClone(G);
    const result = applyTestCommand(clone, playerID, move);
...
function playUntil(G: HegemonyState, done: (G: HegemonyState) => boolean, maxSteps = 200) {
  for (let step = 0; step < maxSteps; step += 1) {
    ...
    expect(applyTestCommand(G, G.currentPlayer, moves[0]).ok).toBe(true);
  }
...
describe("setup enumeration", () => {
  ...
  it("returns an empty list for off-turn players", () => {
    const G = scenario().build();
    expect(enumerateLegalCommands(G, "2")).toHaveLength(0);
  });
...
describe("gameplay enumeration", () => {
  ...
  it("offers multi-pop movePops bundles that apply cleanly", () => {
    ...
    expect(applyTestCommand(structuredClone(G), "0", bundle!).ok).toBe(true);
  });
  ...
  it("drops every costed move when the player cannot afford anything", () => {
    ...
    const types = new Set(enumerateLegalCommands(G, "0").map((move) => move.type));
  ...
  it("enumerates deterministically", () => {
    ...
    expect(enumerateLegalCommands(G, "0")).toEqual(enumerateLegalCommands(G, "0"));
  });

  it("plays whole turns end-to-end through transition", () => {
    const G = openingInGameplay();
...
describe("transition boundary guard", () => {
  ...
  it("rejects an off-turn move (not the current player)", () => {
    ...
    expect(applyTestCommand(structuredClone(G), "1", { type: "endTurn" }).ok).toBe(false);
    // ...but the seat that IS up can act — the guard never over-rejects.
    expect(applyTestCommand(structuredClone(G), "0", { type: "endTurn" }).ok).toBe(true);
  });
  ...
  it("blocks normal moves during a pending riot but allows resolving it", () => {
    ...
    expect(applyTestCommand(structuredClone(G), "0", { type: "endTurn" }).ok).toBe(false);
    expect(applyTestCommand(structuredClone(G), "0", { type: "resolveRiot" }).ok).toBe(true);
  });
