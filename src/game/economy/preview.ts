...
export function previewPlaceSettlement(
  ...
  return previewEconomyAction(G, playerID, `Place ${placementKind}`, (draft) =>
    placementKind === "city"
      ? placeCapital(draft, playerID, tileId, pops)
      : placeColony(draft, playerID, tileId, pops),
  );
}
...
function createSettlementEconomyPreview(
  ...
  const tileIds = [
    ...new Set([
      ...before.map((settlement) => settlement.tileId),
      ...after.map((settlement) => settlement.tileId),
    ]),
  ];
  ...
  return tileIds.map((tileId) => {
    ...
    return {
      ...
      incomeDelta: diffResources(
        next?.income ?? EMPTY_RESOURCES,
        previous?.income ?? EMPTY_RESOURCES,
      ),
